...
// Type definition for web-features
interface WebFeature {
  name?: string;
  description?: string;
  mdn_url?: string;
  spec_url?: string;
  baseline?: boolean | { newly_available: string };
}

// Snapshot data for offline testing - this would be generated from web-features
const SNAPSHOT_DATA: Record<string, WebFeature> = {
  'structured-clone': { 
...
export class DefaultBaselineAdapter implements BaselineAdapter {
  private featureMap: Map<string, WebFeature> = new Map();
  private targetDates: Record<BaselineTarget, Date> = {
  ...
  info(featureId: string): BaselineFeatureInfo | undefined {
    ...
    const result: BaselineFeatureInfo = {
