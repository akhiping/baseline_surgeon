    ...
    traverse(ctx.ast, {
      CallExpression(path) {
        ...
        if (
          ...
          const finding = FindingBuilder.fromBabelNode(
            ctx.filePath,
            node,
            'structured-clone',
            'js.structured-clone',
            ctx.content,
            'Usage of structuredClone() detected'
          );
    ...
    try {
      ...
      if (!hasImport) {
        const importDecl = t.importDeclaration(
          ...
          t.stringLiteral('@baseline-surgeon/polyfills/structured-clone')
        );

        // Add to the beginning of the program
        (ctx.ast as t.File).program.body.unshift(importDecl);
        hasChanges = true;
        changes.push({
          type: 'add',
          line: 1,
      ...
      const result = generate(ctx.ast as t.File, {
        retainLines: true,
        compact: false
      });
